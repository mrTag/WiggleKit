...
func add_particle_to_collider(particle_index:int):
	if Engine.is_editor_hint():
		return
	if _collider_id == -1:
		_collider_id = WiggleKit.sphere_collider_create(position, Radius, Bounciness, Friction)
		WiggleKit.sphere_collider_set_velocity_factor(_collider_id, VelocityFactor)
...
func add_particle_to_sdf_constraint(particle_index:int):
	if Engine.is_editor_hint():
		return
	if _sdf_id == -1:
		_sdf_id = WiggleKit.sphere_sdf_create(position, Radius, SDF_Distance, SDF_Compliance)
...
func _physics_process(_delta):
	if Engine.is_editor_hint():
		return
	if _collider_id != -1:
		WiggleKit.sphere_collider_set_position(_collider_id, position)
		WiggleKit.sphere_collider_set_radius(_collider_id, Radius)
	if _sdf_id != -1:
		WiggleKit.sphere_sdf_set_position(_sdf_id, position)
